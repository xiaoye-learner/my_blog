...
**设置暗色主题**
1. 新建文件theme-switch.vue，之后作为切换组件导入：
```html
<template>
    <el-switch 
        v-model="switchValue"
    ...
    methods: {
        ...
        applyTheme(isLight) {
            if (isLight) {
                ...
                // document.documentElement.style.setProperty('--bg-color', '#EBF5FF')
            } else {
                document.body.classList.add('dark-mode')
                // document.documentElement.style.setProperty('--bg-color', '#31333a')
            }
        },
    }
}
...
```
采用`document.body.classList.add/remove`方法来切换主题色更为便捷，之后在全局css文件中加入`.dark-mode`，用于进行暗色的设置